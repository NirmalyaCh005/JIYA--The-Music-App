...
export default function HomePage() {
  ...
  return (
    <main className="flex-1 flex flex-col select-none">
      ...
      <div className="py-4 sm:py-8 px-3.5 sm:px-6 lg:px-10 w-full max-w-[1700px] mx-auto space-y-6 sm:space-y-10">
        ...
        {searchQuery ? (
          <section className="space-y-6 animate-in fade-in duration-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              >
                <Music2 className="w-5 h-5 sm:w-6 sm:h-6 text-blue-500 shrink-0" />
              ...
              <span
                className={`self-start sm:self-auto text-xs font-mono px-3 py-1 rounded-full border shrink-0 ${
                  isDark
                    ? 'bg-white/10 backdrop-blur-md border-white/15 text-slate-300'
                    : 'bg-white/80 backdrop-blur-md border-slate-200 text-slate-600 shadow-sm'
                }`}
              >
            ...
            {isSearching ? (
              <div
                className={`py-20 text-center rounded-3xl border flex flex-col items-center justify-center gap-3 backdrop-blur-2xl ${
                  isDark
                    ? 'bg-[#151D2A]/70 border-white/10 text-slate-400 shadow-2xl'
                    : 'bg-white/75 border-slate-200/80 text-slate-500 shadow-xl'
                }`}
              >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                <div
                  className={`lg:col-span-2 p-3.5 sm:p-6 rounded-[24px] sm:rounded-3xl border shadow-2xl backdrop-blur-2xl ${
                    isDark ? 'bg-[#151D2A]/70 border-white/10' : 'bg-white/75 border-slate-200/80'
                  }`}
                >
          ...
          <>
            {/* Auto Liked, Recently Played & Live Trending Section */}
            <section className="space-y-3.5 sm:space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <button
                    ...
                    className={`p-2 rounded-full border transition-all active:scale-90 backdrop-blur-md ${
                      isDark
                        ? 'border-white/10 bg-slate-900/40 hover:bg-white/10 text-slate-300'
                        : 'border-slate-200 bg-white/60 hover:bg-slate-100 text-slate-700 shadow-sm'
                    }`}
                    title="Scroll Left"
                  >
              ...
              {/* Dynamic Scrollable Card Carousel (Mobile Phone optimized card sizes) */}
              <div
                ref={scrollContainerRef}
                className="flex items-center gap-3.5 sm:gap-5 overflow-x-auto no-scrollbar pb-3 pt-1 scroll-smooth"
              >
                {dynamicCards.map((item, idx) => (
                  <div
                    key={`${item.track.id}-${idx}`}
                    onClick={() => playTrackList(cardTracksList, idx)}
                    className="group cursor-pointer space-y-2.5 shrink-0 w-36 sm:w-52"
                  >
                    <div className="relative aspect-square rounded-[22px] sm:rounded-[28px] overflow-hidden shadow-xl group-hover:shadow-2xl transition-all duration-300 transform group-hover:-translate-y-1.5 border border-white/15 bg-slate-900/60 backdrop-blur-md">
                      <img
                      ...
                      <div className="absolute top-2.5 left-2.5 sm:top-3 sm:left-3 z-10">
                        <span
                          className={`text-[9px] sm:text-[10px] font-black tracking-wide px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full border backdrop-blur-md shadow-md ${item.badgeBg}`}
                        >
                          {item.label}
                        </span>
                      </div>
                      ...
                      <div className="absolute inset-0 bg-slate-950/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                        <div className="w-11 h-11 sm:w-13 sm:h-13 rounded-full bg-blue-600 text-white flex items-center justify-center shadow-2xl group-hover:scale-110 active:scale-95 transition-transform border border-white/30">
                    ...
                    <div>
                      <h3
                        className={`font-black text-xs sm:text-sm truncate tracking-tight group-hover:text-blue-500 transition-colors ${
                          isDark ? 'text-white' : 'text-slate-900'
                        }`}
                      >
                        {item.track.title}
